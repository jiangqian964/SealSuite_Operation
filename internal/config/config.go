...
// 包含所有子系统的配置项
type Config struct {
	SealSuite SealSuiteConfig `mapstructure:"sealsuite" yaml:"sealsuite"` // SealSuite API 配置
	LLM       LLMConfig       `mapstructure:"llm" yaml:"llm"`             // 大模型配置
	Scheduler SchedulerConfig `mapstructure:"scheduler" yaml:"scheduler"` // 定时任务调度器配置
	Log       LogConfig       `mapstructure:"log" yaml:"log"`             // 日志系统配置
	Server    ServerConfig    `mapstructure:"server" yaml:"server"`       // HTTP 服务配置（预留）
	Database  DatabaseConfig  `mapstructure:"database" yaml:"database"`   // SQLite 数据库配置
}
...
// DatabaseConfig 定义了 SQLite 数据库配置项
type DatabaseConfig struct {
	Path string `mapstructure:"path" yaml:"path"` // SQLite 数据库文件路径
}
